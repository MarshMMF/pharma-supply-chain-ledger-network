  ...
  async addToWallet(user) {
    try {
      ...
      if(!user || user.length<1) {
        ...
        const certPath = path.join(credPath, '/msp/signcerts/User1@org2.example.com-cert.pem');
        ...
        // Remove any existing identity files with various extensions
        try {
            const existingIdentities = await wallet.list();
            for (const id of existingIdentities) {
                if (id.startsWith(`${user}.`)) {
                    console.log(`Removing existing identity: ${id}`);
                    await wallet.remove(id);
                }
            }
        } catch (listError) {
            console.log(`Error listing identities: ${listError}`);
        }
        
        console.log(`Putting identity in wallet with label: ${identityLabel}`);
        await wallet.put(identityLabel, identity);
        console.log(`Identity added to wallet successfully`);
        console.log(`addToWallet mspId:Org2MSP for user: ${user}`);
